...
import com.anubis.loader.entity.pm.InstalledPackage;
...
public class BPackageManager extends BlackManager<IBPackageManagerService> {
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
        ...
        String t1 = intentToResolve.resolveTypeIfNeeded(host.getContentResolver());
        List<ResolveInfo> queryIntentActivities = queryIntentActivities(intentToResolve, 0, t1, userId);
        ...
        if (queryIntentActivities == null || queryIntentActivities.size() <= 0) {
            ...
            intentToResolve.setPackage(packageName);
            String t2 = intentToResolve.resolveTypeIfNeeded(host.getContentResolver());
            queryIntentActivities = queryIntentActivities(intentToResolve, 0, t2, userId);
        }
        if (queryIntentActivities == null || queryIntentActivities.size() <= 0) {
            return null;
        }
    ...
    public ProviderInfo resolveContentProvider(String authority, int flags, int userId) {
        IBPackageManagerService svc = getService();
        if (svc == null) {
            return null;
        }
        try {
            return svc.resolveContentProvider(authority, flags, userId);
        } catch (RemoteException e) {
            crash(e);
        }
        return null;
    ...
    public List<ResolveInfo> queryIntentActivities(Intent intent, int flags, String resolvedType, int userId) {
        ...
        try {
            return svc.queryIntentActivities(intent, flags, resolvedType, userId);
        } catch (RemoteException e) {
            crash(e);
        }
        return Collections.emptyList();
