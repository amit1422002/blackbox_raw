...
import com.anubis.loader.entity.UnbindRecord;
...
public class BActivityManagerService extends IBActivityManagerService.Stub implements ISystemService {
    ...
    @Override
    public String getLaunchedFromPackage(IBinder token, int userId) throws RemoteException {
        UserSpace userSpace = getOrCreateSpaceLocked(userId);
        synchronized (userSpace.mStack) {
            return userSpace.mStack.getLaunchedFromPackage(token, userId);
        }
    }
