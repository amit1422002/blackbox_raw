...
import com.anubis.util.ContextUtil.openAppSystemSettings
import com.anubis.view.main.ShortcutActivity
...
object ShortcutUtil {
    ...
    private fun showAllowPermissionDialog(context: Context){
        if (!AppManager.mAnubisLoader.showShortcutPermissionDialog()){
            return
        }
        ...
        MaterialDialog(context).show {
            ...
            neutralButton(R.string.no_reminders){
                AppManager.mAnubisLoader.invalidShortcutPermissionDialog(false)
            }
        }
