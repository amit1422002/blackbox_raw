...
const char *IO::redirectPath(const char *__path) {
    ...
    const char *bestTarget = nullptr;
    const char *bestRelocate = nullptr;
    size_t bestLen = 0;
    ...
    for (iterator = relocate_rule.begin(); iterator != relocate_rule.end(); ++iterator) {
        ...
        if (__path[targetLen] != '\0' && __path[targetLen] != '/') {
            continue;
        }
        bestTarget = info.targetPath;
        bestRelocate = info.relocatePath;
        bestLen = targetLen;
    }

    if (bestTarget != nullptr) {
        char *ret = replace(__path, bestTarget, bestRelocate);
        return ret;
    }
    return __path;
}
