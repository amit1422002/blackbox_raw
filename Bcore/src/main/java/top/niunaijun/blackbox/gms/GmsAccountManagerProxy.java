...
/**
 * GmsAccountManagerProxy — strongly based on zcore's eh.java.
 *
 ...
 *     → Return KEY_INTENT pointing to GoogleSignInActivity
 *     → GMS frameworks handle KEY_INTENT correctly (they spawn the activity
 ...
 *     → Return onError(CANCELED) immediately to unblock the caller
 *     → startActivity(GoogleSignInActivity) independently as new task
 *     → BroadcastReceiver picks up ACTION_GSI_TOKEN → bridges the result back
 *     → Avoids the Play Store freeze/deadlock
 */
public class GmsAccountManagerProxy extends BinderInvocationStub {
    ...
    /** When microG/real GMS is installed, delegate to its AccountAuthenticator. */
    static boolean useMicroGAuth() {
        try {
    ...
    public static class getAuthToken extends MethodHook {
        @Override protected Object hook(Object who, Method m, Object[] a) throws Throwable {
            ...
            if (TextUtils.isEmpty(tok)) tok = tryHostBridge(acct, tt);
    ...
    public static Object handleGoogleAddAccount(IAccountManagerResponse resp, String tokenType) {
        try {
            ...
            if (ex != null) {
                ...
                if (TextUtils.isEmpty(tok)) tok = tryHostBridge(ex, tokenType);
                if (!TextUtils.isEmpty(tok)) {
    ...
    private static Intent buildSignInIntent(Context ctx, String tokenType) {
        ...
        intent.setComponent(new ComponentName(BlackBoxCore.getHostPkg(),
                "top.niunaijun.blackbox.gms.GoogleSignInActivity"));
    ...
    private static String tryHostBridge(Account acct, String tokenType) {
        Context ctx = BlackBoxCore.getContext();
        if (ctx == null) return null;
        return HostAccountBridge.fetchHostToken(ctx, acct, tokenType);
    }
