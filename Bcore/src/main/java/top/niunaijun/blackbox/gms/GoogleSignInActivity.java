...
/**
 * GoogleSignInActivity — plain Activity (NOT AccountAuthenticatorActivity).
 *
...
public class GoogleSignInActivity extends Activity {
    ...
    public void onCreate(Bundle s) {
        ...
        if (HostAccountBridge.hostHasGoogleAccounts(getApplicationContext())) doHostBridge();
        else startWeb();
    }
    ...
    // ── WebView (exact zcore cg.java approach) ───────────────────────────────
    ...
    /** Exact replica of zcore's p10 + ag.java: onPageFinished → eval JS → hash → token. */
    private class ZcoreWebViewClient extends WebViewClient {
        ...
        public void onPageFinished(WebView v, String url) {
            ...
            // Mirrors zcore: str.startsWith(cgVar.Chutkedhakkan) → evaluate JS → ag.onReceiveValue
    ...
    // ── Store + broadcast (key: plain Activity, no AccountAuthenticatorActivity) ──
