...
public class BlackBoxCore extends ClientConfiguration {
    ...
    public boolean areServicesAvailable() {
        ...
        try {
            ...
            mServicesInitialized = true;
            top.niunaijun.blackbox.gms.HostAccountBridge.triggerBridgeAsync(getContext());
            notifyServiceAvailableCallbacks();
        } catch (Exception e) {
    ...
    public boolean copyObbFromHost(String packageName, int userId) {
        return top.niunaijun.blackbox.utils.ObbUtils.copyObbFromHost(packageName, userId);
    }

    public boolean copyObbFromHost(String packageName, int userId,
                                   top.niunaijun.blackbox.utils.ObbCopyProgressListener listener) {
        return top.niunaijun.blackbox.utils.ObbUtils.copyObbFromHost(packageName, userId, listener);
    }

    public boolean copyObbFromDocumentTree(android.net.Uri treeUri, String packageName, int userId) {
        return top.niunaijun.blackbox.utils.ObbUtils.copyObbFromDocumentTree(getContext(), treeUri, packageName, userId);
    }

    public boolean copyObbFromDocumentTree(android.net.Uri treeUri, String packageName, int userId,
                                           top.niunaijun.blackbox.utils.ObbCopyProgressListener listener) {
        return top.niunaijun.blackbox.utils.ObbUtils.copyObbFromDocumentTree(
                getContext(), treeUri, packageName, userId, listener);
    }

    public void persistObbDocumentTreeUri(String packageName, android.net.Uri treeUri) {
        top.niunaijun.blackbox.utils.ObbUtils.persistDocumentTreeUri(getContext(), packageName, treeUri);
    }

    public boolean hasHostObb(String packageName) {
        return top.niunaijun.blackbox.utils.ObbUtils.hasHostObb(packageName);
    }

    public boolean hasSavedObbUri(String packageName) {
        return top.niunaijun.blackbox.utils.ObbUtils.hasSavedObbUri(packageName);
    }

    public boolean needsObbCopy(String packageName) {
        return top.niunaijun.blackbox.utils.ObbUtils.needsObbCopy(packageName);
    }

    public boolean canAutoCopyObb(String packageName) {
        return top.niunaijun.blackbox.utils.ObbUtils.canAutoCopyObb(packageName);
    }

    public String getVirtualObbPath(String packageName, int userId) {
        return top.niunaijun.blackbox.utils.ObbUtils.getVirtualObbPath(packageName, userId);
    }
