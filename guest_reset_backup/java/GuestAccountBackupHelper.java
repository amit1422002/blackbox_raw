...
/**
 * Saves BGMI guest reset dumps + device_id backups under
 * {@code /storage/emulated/0/anubisloader/guest_accounts/}.
 */
public final class GuestAccountBackupHelper {
    ...
    /** @return backup folder path, or null on failure */
    public static String saveBackupAndDump(Context context, GuestSnapshot snap, String newUuid) {
        try {
            ...
            String dump = buildDumpText(context, snap, newUuid, backupDir.getAbsolutePath());
            File dumpFile = new File(backupDir, "reset_dump.txt");
            writeUtf8File(dumpFile, dump);
    ...
    public static String getPublicBackupHint() {
        return Environment.getExternalStorageDirectory().getAbsolutePath()
                + "/" + ROOT_FOLDER + "/" + GUEST_ACCOUNTS_FOLDER;
    }
    ...
    private static String buildDumpText(Context context, GuestSnapshot snap, String newUuid,
                                        String backupPath) {
        ...
        sb.append("Package: ").append(CloneDataHelper.BGMI_PKG).append(" (loader clone)\n");
