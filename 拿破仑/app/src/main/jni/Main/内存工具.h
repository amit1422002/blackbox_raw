...
ssize_t process_v(pid_t __pid, const struct iovec * __local_iov,
    ...
    int pvm(void* address, void  *buffer,size_t size) 	// 读取核心函数
    {
        ...
        remote[0].iov_len = size;
           ...
           return (int)__syscall6(__NR_process_vm_readv, (long)pid, (long)local, (long)1, (long)remote, (long)1, (long)0);   
            
    
    }
    
    int pvm2(void* address, void  *buffer,size_t size) 	// 读取核心函数
    {
        struct iovec local[1];
        struct iovec remote[1];
...
// 进程读写内存
size_t preadv(long int address, void *buffer, size_t size)
{
...
size_t pwritev(long int address, void *buffer, size_t size)
{
...
bool vm_readv(long int address, void *buffer, size_t size) {
    return pvm(reinterpret_cast < void *>(address), buffer, size);
}


/* 写内存 */
bool vm_writev(long int address, void *buffer, size_t size) {
    return pvm2(reinterpret_cast < void *>(address), buffer, size);
}
...
int SetingSize(int i)
{
	isSize = i;
	return 0;
}

uintptr_t getZZ(unsigned long addr)
{
	if(Check(addr) == 1)
	{
...
void WriteFloat(int pid,long int addr, float value)
{
    char lj[64];
    int handle;
...
void WriteDword(int pid,long int addr, int value)
{
    char lj[64];
    int handle;
    sprintf(lj,"/proc/%d/mem", pid);
